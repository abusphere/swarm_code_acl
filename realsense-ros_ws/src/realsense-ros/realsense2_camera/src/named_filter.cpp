...
void PointcloudFilter::Publish(rs2::points pc, const rclcpp::Time& t, const rs2::frameset& frameset, const std::string& frame_id)
{
    ...
    use_texture = false; //NOTE: Set use_texture to false to allow black and white depth cloud
    if (use_texture)
    {
    ...
    else
    {
        ...
        for (size_t point_idx=0; point_idx < pc.size(); point_idx = point_idx + 10, vertex = vertex + 10)
        {
            bool valid_pixel(vertex->z > 0 && vertex->z < 6.0);
            if (valid_pixel || _ordered_pc)
            {
                *iter_x = vertex->x;
                *iter_y = vertex->y;
...
bool is_enabled = true; //Used to get rid of color image. Probably don't need anymore.
